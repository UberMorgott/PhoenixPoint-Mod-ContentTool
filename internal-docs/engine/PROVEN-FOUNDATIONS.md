...
| U3b/U3c | **IN-GAME GREEN 2026-09-01.** A shaderless generated Material is handed a shader off a LIVE object at runtime (U3b, FINAL-PLAN §10.1/§10.2), and the property block baked before any shader existed survives being interpreted for the first time (U3c). The donor is not `Shader.Find` — that would answer "does this process have a Standard shader" — but the Shader instance U3d just resolved out of the game's own `defaultlocalgroup_unitybuiltinshaders.bundle`, the same object a Def walk ends at, reached without loading an actor prefab. U3c's texture oracle is object IDENTITY against the Texture2D U1 asserted the pixels of, so a shader that merely exposes a `_MainTex` slot cannot pass it; U3b's control is the SAME material read BEFORE the assignment, so "it says Standard" cannot be something it was already saying | `autogate.ps1 -Commands ct_bake,ct_project` 2026-09-01, build `9593be32` confirmed in-phase: `U3b PASS the shaderless baked Material reported 'Hidden/InternalErrorShader', and after being handed the live donor's shader reports 'Standard'` · `U3c PASS after the assignment _MainTex -> 'selfcheck' isTheBakedTexture2D=True \| _Glossiness=0,25` · `ct_bake: ALL PASS`. Impl: `BakeSelfCheck` U3b/U3c arm, after U3e so `U3d-ctl-noptr` still reads the material unassigned. Ceiling: the donor is reached through a shipped bundle rather than a Def walk — the mechanism under test is the assignment, not the lookup |
...
- **How to TRIGGER it — CLOSED 2026-09-01, the natural screen is now MEASURED.** The console
  ...
  - **2026-09-01: the POST is now measured headlessly, the SCREEN is still unverified.**
    ...
    the whole decompile, still no `file:line` to cite. **Superseded in part by the entry below:
    the WIRING is now proven; only the owning screen is still open.**
  - **2026-09-01 — the BINDING is measured, the SCREEN is still UNVERIFIED (bounded).** The dig
    that "zero hits in the decompile" ended is finished: **the event is not wired by id in a
    prefab's Wwise component at all, it is wired by a DEF.** The whole seam, in order:
    - **The consumer is `Base.Audio.UIButtonSounds`** (`decompiled\AssemblyCSharp\Assembly-CSharp\
      src\Base.Audio\UIButtonSounds.cs:10`) — a `MonoBehaviour` that takes SIX `BaseEventDef`
      fields (`Click`/`Enter`/`Exit` and their `…Disabled` twins, `:13-24`), attaches itself to the
      parent `Button`/`Toggle` (`:36-45`) and on click calls
      `EventusManager.PlayEventDirect(eventDef, this)` (`:167`). The def carries the sound as
      `Base.Audio.AudioEventData.Event` (`AudioEventData.cs:11`, type `AK.Wwise.Event`). So an
      event NAME can never appear in code — the name lives in a def asset. That is the reason for
      the zero hits, and it is now a measured reason rather than a hypothesis.
    - **The def is `StatPlusClickEventDef`** — `PhoenixPoint.Common.UI.UIEventDef`, guid
      `3b74bb54-47ae-b1f4-f88d-f6f1f7360c85`, `ResourcePath = Defs/Eventus/UI/StatPlusClickEventDef`,
      and its own `Name` field reads **`StatConfim`** (the game's typo, not ours). Read LIVE off the
      running game on `D:\PP-Instance2` (PPCLI, build `c9ea2c6c`, 2026-09-01):
      `AudioData.Event.Id` -> **`784388130`** and `AudioData.Event.Name` -> **`GUI_StatsPlusClick`**.
    ...
    - **Which button holds that def — answered one entry below; this is how the search narrowed.**
      ...
      **813** loaded `UIButtonSounds` (`Resources.FindObjectsOfTypeAll`), every one's `Click` read
      ...
      `MenuClickEventDef`, 7 `ManufacturingEnterEventDef`, 4 `MainMenuClickEventDef`, 4
      `SoldierClickEventDef`, 3 `WepWheelClickEventDef`, 1 `AbilityClickEventDef`, 1
      `MainMenuEnterEventDef`; the rest have no `Click`). So the owner is on a GEOSCAPE screen
      prefab that a tactical mission never loads, and pinning it needs a live geoscape — which was
      not started, because the only running game was the author's own live mission.
    - **Also ruled out, so nobody repeats it:** the game ships NO `AkEvent` and NO `AkAmbient`
      components (`FindObjectsOfTypeAll` -> `count:0` for both, live) — PP does not use the Wwise
      Unity MonoBehaviours at all; `Base.Audio\WwiseIDs.cs` is a STALE generated table (uppercase
      names from an older Wwise project, no `GUI_` prefix, and `784388130` is not in it); the 999
      loaded `WwiseObjectReference` assets are the project-wide auto-generated table, so the name
      appearing once in `sharedassets0.assets` proves nothing about use; the complete list of every
      `AK.Wwise.Event` FIELD in `Assembly-CSharp` (~110, swept by regex) contains no stat/skill/
      generic-click field, and `Assembly-CSharp.dll` is the only game assembly shipped
      (`PhoenixPointWin64_Data\Managed\`), so there is no second place to look. `GeoscapeSoundDef`,
      `GamestateSoundDef`, `UIModuleBattleSummary` (which owns the neighbouring `StatXPBangup*` /
...
| # | Closed fact | Evidence |
|---|---|---|
| U8-grid | **IN-GAME GREEN 2026-09-01** (was PENDING-INGAME). The uniform clip grid must REACH the last key. `GlbReader.Animation` sized it `Round(duration * rate) + 1`, which (a) put the last frame BEFORE the last key whenever that key sits in the front half of a frame — 0.511 s at 30 Hz ended the clip at 0.500 s and threw the last 11 ms of every curve away — and (b) wrapped a finite but enormous timestamp to `int.MinValue` on the cast, sailing under the `MaxFrames` guard and falling through the `frames < 2` floor into a SILENT two-frame clip. Now `Ceiling(duration * rate - GridTolerance)` with the frame ceiling compared as a **double, before any cast**, so a huge duration is REFUSED BY NAME. `GridTolerance` is the 0.01 frame `Rate()` already used as a literal — ONE number decides both "this key is on the grid" and "this many frames reach it", so a snapped clip keeps exactly the frame count it always had | `dotnet run --project tests\ObjCodecTests` (`ClipImport.Grids`), commit `c58f3be`: 17 frames, last frame 0.533 s ≥ last key 0.511 s. Falsified twice — `Round` → the clip ends at 0.5; a post-cast guard → `OverflowException` instead of a refusal by name. **Lives only on the FALLBACK path** (no whole rate in [1, 120] Hz lands on every key), which is why no test in this repo had ever executed it: all five spider clips snap at 24 Hz. The fixture is hand-built and had to be — Dirichlet's approximation theorem makes a SINGLE key time always snap, so falsifying it needs two incommensurate times (0.1 / 0.511) |
| U8-step | **IN-GAME GREEN 2026-09-01** (was PENDING-INGAME). A STEP curve's HOLD survives to within one frame of its jump. A dense bank cannot hold a discontinuity, so a STEP channel resampled at the clip's own snapped rate smears the jump over a whole frame of that rate. `GlbReader.StepGrid` resamples a clip carrying any STEP channel on the **highest whole multiple of its own snapped rate the mod allows**, so the forced ramp is at most one frame at up to 120 Hz — 8.333 ms — and the bound is PRINTED in `SampledClip.LossyReason` rather than left for the author to discover | `dotnet run --project tests\ObjCodecTests` (`ClipImport.HandBuilt`), commit `c3be088`: 121 frames @ 120 Hz, every frame an exact key value, the hold reaching frame 59, ramp 8.333 ms. Falsified by disarming `StepGrid` → 250 ms. Ceiling: an EXACT hold needs a `StreamedClip` keyframe bank, which this writer does not have |

**Closed in the game 2026-09-01, together with `U9-plan`, by the fixture they were missing.** The
blocker was named correctly and it was real: no shipped file and no demo reaches these paths — all
five clips of the spider probe snap onto a whole rate, none is a STEP curve, and all five are
uniquely named and drive bones — so the in-game arms had nothing to read. The fixture is now a REAL
FILE: `lib\u9_probe.glb` (2 888 B), assembled out of `ClipImport`'s own `Bin`/`Container`/`Sampler`
by `dotnet run --project tests\ObjCodecTests -- --u9probe <out>` and committed, with the csproj
putting it beside the DLL the way `u8_probe`/`u10_probe` already are. It carries the three shapes in
one file: `Walk` (LINEAR, keys 0 / 0.1 / 0.511 s — no whole rate up to 120 Hz serves both), `walk`
(the same curve on the hip, the same name once lowercased), `Morphs` (a blend-shape channel plus a
node the armature does not list, so zero tracks) and `Hold` (STEP, keys 0 / 0.5 / 1 s).
`BakeSelfCheck` reads it, plans it, and BAKES its two interesting clips into the self-check bundle,
so each row has two readings that fail apart — what this mod's resampler made of the file, and what
the ENGINE then made of the clip baked from it. `autogate.ps1 -Commands ct_bake` 2026-09-01, build
`e9e78a6f` confirmed in-phase:
`U8-grid PASS … reader says 30 Hz x 17 frame(s), last frame 0.533 s, and the engine reads the baked clip back as 30 Hz, 0.533 s (… rounding would end at 0.5)` ·
`U8-step PASS … reader says 120 Hz x 121 frame(s), holds to frame 59, ramp 8.333 ms … and the engine reads the baked clip back as 120 Hz, 1 s (… the 2 Hz LINEAR grid would ramp 250 ms)` ·
`U9-plan PASS … planned u9_probe_walk, u9_probe_walk_1, u9_probe_hold | clip 'Morphs' drives no bone of this rig and was SKIPPED - the model's other clips are unaffected` ·
`ct_bake: ALL PASS`. A missing fixture reports `U9-plan VOID the fixture is gone`, never a silent
skip — the SKIN-ABOVE mistake, not repeated.
...
| U12 | **IN-GAME GREEN 2026-09-01** (was PENDING-INGAME). **A baked clip LOOPS through `m_MuscleClip.m_LoopTime`, and through nothing else.** MEASURED over all **650** `AnimationClip`s of nine shipped bundles: `m_WrapMode` is **0 on every one of them**, looping or not (it is the legacy `Animation` component's field; these are `m_Legacy=false`), while `m_LoopTime` is true on **132** and false on **518** — `px_equipment` ships `Turret_ShootLoop` (true) beside `Turret_ShootEnd` (false), both wrap 0. Nothing travels with it: `m_CycleOffset` 0 on all 650, `m_StartAtOrigin` true on all 650, `m_LoopBlend` true on only 20 — every one of those already `m_LoopTime=true`, so it is an extra, not a requirement. glTF carries **no loop flag** (checked: the probe's only `extras` are material entries), so the AUTHOR declares it: one optional `ppcontent.json` string `"loop": "Spider_Idle, Spider_Walk"`, naming the `.glb`'s own clip names, case-blind | `dotnet run --project tests\ObjCodecTests`, commit `c057113`: a SHIPPED loop/one-shot PIN plus both directions asserted on the five baked spider clips (`ClipBake.cs`), and the parse plus the refusal (`ClipPlan.cs`). A name no model carries is a bake FAILURE (`clip-names`) that lists the names the project DOES carry — never a silent no-op. Falsified three times: writing `m_WrapMode=2`; hardcoding the flag; making the name match case-sensitive. **IN-GAME 2026-09-01, `D:\PP-Instance2`, ContentTool build from commit `0e22250`:** `ct_project CustomCreature` re-baked and `ct_creature gate spider_demo_before customcreature` spawned the actor; the ENGINE's own deserialised clips were then read off the live actor's `AnimatorOverrideController` (`Overridden: HumanoidAnimatorLOC`, 73 clips) through PPCLI reflection, and the split is exactly the declaration's: **`cyborg_spider_spider_idle isLooping=True`** and **`cyborg_spider_spider_walk isLooping=True`** (the two names `"loop": "Spider_Idle, Spider_Walk"` carries) against **`cyborg_spider_spider_attack_1` / `_attack_2` / `_death` `isLooping=False`** (declared by nobody) — with **`wrapMode=Default` on every one of them**, which is the row's "and through nothing else" measured in the live engine rather than over shipped bundles. The synthesised climb clips split the same way in the same read: `cyborg_spider_ct_climb_loop isLooping=True` against `ct_climb_start` / `ct_climb_stop isLooping=False`. The gate's own arms agree from inside the game: `ct_creature PASS role 'idle' = 'cyborg_spider_spider_idle' isLooping=True`, `ct_creature PASS role 'walk' = 'cyborg_spider_spider_walk' isLooping=True`, `ct_creature PASS climb loop 'cyborg_spider_ct_climb_loop' isLooping=True`. **VISUAL PASS DONE 2026-09-01** — the author watched the two demo spiders (`Swarmer_33` / `Swarmer_34`) in a live mission on map `ALN_PLT_Nest_48x48_A` and reports the idle animation plays and loops calmly. So `U12` is now proven BOTH by the engine's `isLooping` flag and by eye. Bounded: this confirms the IDLE loop as seen on screen, nothing more — it says nothing about the other clips, about blending, or about the loop being seamless at the sample level |
| U13 | **IN-GAME GREEN 2026-09-01** (was PENDING-INGAME). **The author picks WHICH clip the baked `Animator` plays**: optional `ppcontent.json` `"play": "Spider_Walk"`, the `.glb`'s own clip name, case-blind — absent = the first bakeable clip, which is what every bake did before. `ClipFields.Chosen(plan, play)` is the ONE resolver, used both by the bake (which clip the AOC overrides) and by the `U9`/`U9-mecanim` drive arms (whose samples the rig is asserted against), so the two cannot disagree | `dotnet run --project tests\ObjCodecTests`, commit `93f3901`: the AOC's override clip read back off the FILE by name in `ClipBake.cs`, with `"play"` deliberately NOT the file's first clip, and `'Walk'`/`'walk'`/unbakeable `'Morphs'` in `ClipPlan.cs`. A declared name that is not in the plan returns **-1, never a silent fallback to clip 0**; the project-level `clip-names` arm validates `"loop"` and `"play"` against the BAKEABLE clip names of every model and FAILS by name, which also catches declaring a clip that drives no bone and is therefore never written. Falsified by returning 0 for every match, and by falling back to 0 instead of -1. **IN-GAME 2026-09-01, `D:\PP-Instance2`:** `ct_project CustomCreature` ran inside the live game and the ENGINE's own Animator drove the declared clip — `U9-mecanim PASS the file's own clip 'cyborg_spider_spider_idle' by Animator.Update(0.833) through 'cyborg_spider_aoc' (AnimatorOverrideController) puts every bone of the IMPORTED rig where the .glb says at frame 25 of 51: 47 bone(s) checked, 0 off by more than 0.001, 42 of them off their rest pose (furthest 'lapa_4_L_5_025' travelled 58.241)`, with `clip-names PASS "loop" names 2 clip(s) and "play" names 1 of the 7 this project bakes` and the bake line `clip 'cyborg_spider_spider_idle' … ANIMATOR PLAYS THIS`. **The discrimination is that `"play": "Spider_Idle"` is NOT the first bakeable clip** — the plan leads with `Spider_Walk` — so an AOC that had silently fallen back to clip 0 would have been asserted against idle's samples and gone red; 42 bones off rest is the same run's proof that the clip evaluated at all. Ceiling: **ONE clip per model** — a state machine that switches Idle/Walk needs a `ControllerConstant`, which this route does not serialize |
...
| ZW6 | **RESTART: all four routes come back by themselves** after a clean exit and relaunch with no mod-manager interaction. `c1242d4` startup pass; in the measured runs the `SetEnabled` postfix restored them. The startup ON-pass is the belt for a content mod that does not declare ContentTool as a dependency — **that path is EXERCISED as of 2026-09-01**: `demos\NoDepTexture` (no `Dependencies` key at all) came back on a cold launch through the startup reconcile alone, `1/1 bundle(s) redirected LIVE for 'morgott.demo.nodeptexture'`, see the frontier list below | 2026-08-27, belt 2026-09-01 |
...
~~`U3b`/`U3c` donor shader~~ (closed above, in game, 2026-09-01 — it was optional in the baked path
since U3d gives the shader at bake time, and it still stands for anything materialized live) ·
...
- ~~**The startup ON-pass for a content mod that does NOT declare ContentTool as a dependency** is
  unexercised (ZW6's belt path).~~ — **closed 2026-09-01.** `demos\NoDepTexture` (`meta.json` carries
  no `Dependencies` key at all, by design) was ticked on in `MOD_ACTIVATED` and the game cold-launched
  on `D:\PP-Instance2`. `Player.log`: `ct_content: 'morgott.demo.nodeptexture' is ON in the mod
  manager, so its live registrations were installed at startup.` carrying its own report —
  `installing 1 patched copy(ies) as 'morgott.demo.nodeptexture'` ·
  `redirected aln_acidworm_assets_all.bundle -> …\ContentTool\Patched\d29f58a2\morgott.demo.nodeptexture\aln_acidworm_assets_all.bundle for 'morgott.demo.nodeptexture', crc 1151466550 -> 0 (in memory)` ·
  `1/1 bundle(s) redirected LIVE for 'morgott.demo.nodeptexture' - nothing was written to the game
  installation`. The startup RECONCILE pass is what installed it: the mod's own mid-session postfix
  line (`'morgott.demo.nodeptexture' was switched ON in the mod manager`) carries no bundle at all.
...
- ~~**Everything still filed PENDING-INGAME above** — `U8-grid`, `U8-step`, `U9-plan`,
  `U9-verdict`.~~ — **all four closed 2026-09-01.** The first three by the fixture they were
  missing (`lib\u9_probe.glb`, see the paragraph under the U8 table); `U9-verdict` on its PASS path
  only, with the FAIL branches named as a ceiling in its own row. Nothing is filed PENDING-INGAME
  any more. Offline green is not this document's standard; a row leaves PENDING-INGAME when the
  ...
  flag), `U13` (chosen clip) and `Q1` (who quit the game) left it on **2026-09-01** — see their rows
  for the readings: the engine's own `isLooping` split across declared and undeclared clips of one
  live actor, `U9-mecanim` driving a `"play"` that is deliberately not clip 0, and a `Player.log`
  carrying `Q1-exit` 3,0 s after `Q1-play` with `Q1-watchdog … are up` absent.
