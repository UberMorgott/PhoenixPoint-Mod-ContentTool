...
- [ ] **Step 2: The seam — a JSON-STRING contract, because PPCLI cannot return anything else** (blocker finding 3).
  ...
  `busy`, `stage`, `cancelRequested`/`cancelAcknowledged`, `failedMember`, `claimHeld`, `barrierParked`, `barrierRunId`,
  ...
  **The run-handle protocol:** `Run` returns the accepted `runId`; every subsequent poll compares
  ...
  `barrierParked` is published with `barrierRunId` **only when the worker is actually parked**, never on arming alone —
...
- [x] **Step 1: Preflight, in this order.**
...
- [x] **Step 2: The helpers.** `D`, `C`, `Shot` exactly as design §8.2 (`…-design.md:427`–`:445`) spells them; RPC and
...
- [x] **Step 3: Run the suite in this order** — `W8 → W9 → W10 → W16 → W11 → W13 → W20 → W19a → W19b → W14 → W12 →
...
- [x] **Step 4: Fill the table below** with the actual calls made and the actual strings returned.
...
**THE RUN, 2026-09-06.** ContentTool at HEAD `52178e5`, deployed by the repo's own `deploy.ps1` (default target
`D:\PP-Instance2`) → `D:\PP-Instance2\Mods\ContentTool\ContentTool.dll` (1 972 224 B, 06.09.2026 12:23). The
acceptance marker `D:\PP-Instance2\Mods\ContentTool\ct-acceptance-enabled` was created for this run. PPBridge was
NOT redeployed — `build=3068ae67` in every reply, no `stale:true` anywhere. `com.morgott.ContentTool`,
`com.morgott.PPBridge` and `Replace_Leftleg` were already in the Instance2 profile's `MOD_ACTIVATED`
(`…\Steam\76561197996210592\Options.jopt`, count 19 matching `ArrayDimensions.CollectionValues`) — **the profile was
never edited**. Launched BY HAND:
`Start-Process 'D:\PP-Instance2\PhoenixPointWin64.exe' -ArgumentList '-mods','-logFile','D:\PP-Instance2\ct-t8.log'`
(run 1, PID 20740) and `…ct-t8b.log` (run 2 after the restart, PID 12424). Gate: `connect state -PPRoot
'D:\PP-Instance2' -ProfileId 76561197996210592` answered `{"phase":"menu","scene":"BaseScene"}` before anything else
was sent, in both runs. Identity preflight: the seam's `gameRoot` read `D:\PP-Instance2` in both runs. Geoscape from
`plans\start-campaign.json` (37 steps, 14.4 s / 15 s), then `ct_bench open` + `FitBench.tab = 2`. Screenshots are in
...
- **D1 — the row verdict is drawn unbounded, and it destroys the panel.** `LifecycleDashboard.cs:677`
  ...
  (`verdictLength` measured **1 225 445** on `DashboardValid`), so after any Bake the Apply/Verify/Package rows,
...
`Mods\DashboardValid`, `Mods\DashboardPatchFail`, `Mods\DashboardAuthor`, `Mods\DashboardResident` and
`Mods\Replace_CHR_AN_Assault_M_Leftleg` (W17's real SHIP output), plus their patched copies under
`…\ContentTool\Patched\d29f58a2\` and two packages under `%LOCALAPPDATA%\ContentTool\Packages\`.

| Row | Calls (from design §8.2) | Required evidence | Result |
|---|---|---|---|
| W8 empty | `Open('')`; `Run('Validate')`; `Snapshot`; `Shot 'W8'` | Open-empty allowed; R25. Five `never / —` rows, placeholders, unavailable actions disabled, no layout exception |**PASS** 2026-09-06. `Open("")` -> `{"ok":true,"root":"","id":""}`; `Run("Validate")` -> `{"ok":false,"runId":0,"refusal":"Lifecycle: select a ContentMods project."}` (R25). Header: five rows `never / none / 0 / len 0`. `W8.png`: Project `(none)`, Session `Ready.`, five rows `never none - -`, Progress `-`, Cancel GREY, Log tail `-`, no layout exception. |
| W9 selector/Validate | `Acceptance('prepare')`; `Open('DashboardValid')`; `Run('Validate')`; `Snapshot`; `Shot 'W9'` | **Validate producer lands in T5b.** Disabled fixture included, exact root bound, S3; eligibility shown as its own field and `Disabled` blocks nothing; prev/next/Refresh share the selection path; duplicate name disambiguated by root |**PASS** 2026-09-06. `Acceptance("prepare")` -> ok; `Open("DashboardValid")` bound the exact root `D:\PP-Instance2\Mods\DashboardValid` / id `acceptance.dashboardvalid`. S3 verbatim: `Validate: PASS - 'DashboardValid' - key 2951e52eae3a4a8704a2d1bd5e2990c31f5cb808.` The fixture is NOT in `MOD_ACTIVATED` (disabled) and Validate still PASSes - eligibility is `StageReport.Eligibility` (StageValidate.cs:47-49), its own field, and blocks nothing. Refresh (`rescan=true`, the button's own entry at LifecycleDashboard.cs:641) grew the list 19 -> 22 and it now holds `DashboardAuthor`, `DashboardPatchFail`, `DashboardValid`; duplicate names disambiguated by root: `IntroVideo  [D:\PP-Instance2\Mods\ContentTool\IntroVideo]` vs `IntroVideo  [D:\PP-Instance2\Mods\IntroVideo]` (same for `MenuMusic`). `W9.png`. NOTE: eligibility is not painted as a panel column and is not on the `Snapshot` wire - only the chain reads it. |
| W10 happy chain | enable `DashboardValid` in the mod manager; `Open('DashboardValid')`; `Run('All')`; `Shot 'W10-running'`; `Snapshot`; `Shot 'W10'` | **Validate/Apply/Verify producers land in T5b** — before it, three of these five rows cannot report at all. Clean process, target not resident: five rows PASS, Apply S2, exact producer strings, Package writes a new external path. A resident target makes this W12, not W10. **The fixture is ENABLED** — Verify's live half needs it (W9's disabled-fixture arm is about listing, not verifying). This install is what makes `DashboardValid` un-re-bakeable for the rest of the process; every later baking row uses `DashboardAuthor` |**PASS** 2026-09-06, on the SECOND attempt. First `Run("All")` (runId 2-4) stopped at Apply VOID: `REFUSED: mod 'Replace_Leftleg' already replaces an_assault_assets_all.bundle - 'acceptance.dashboardvalid' cannot also replace it.` - the fixture SOURCE was enabled and held the claim (bench mechanics, see D5). After `Route7.Toggle("D:\PP-Instance2\Mods\Replace_Leftleg", false)` -> `stopped redirecting an_assault_assets_all.bundle for 'Replace_Leftleg', crc restored to 2109966580`, `Run("All")` (runId 5-9) gave FIVE PASS, all `fresh`. Apply S2 verbatim: `1/1 bundle(s) redirected LIVE for 'acceptance.dashboardvalid' - nothing was written to the game installation`. Verify: `Verify: PASS - load-back gates passed; 1 of 1 declared target(s) served from this project's copies for 'acceptance.dashboardvalid'.` Package: `PACKAGED 3 file(s), 4270265 B into C:\Users\Morgott\AppData\Local\ContentTool\Packages\acceptance.dashboardvalid\20260906-123543-9` (a NEW external path). `W10-running.png` (Session `Finishing: Bake`, FIT/MODEL DOCTOR/Refresh/Run all GREY while owned), `W10.png`. **Deviation:** the fixture was NOT enabled in the mod manager - Verify's live half is the per-target claim census (`ReadBack.cs:148-155`), which the Apply itself satisfies; no activation was needed. **`Snapshot("s1s2")` is `null/null` after a dashboard Apply by design** (Harvest clears it, LifecycleDashboard.cs:522); S2 lives in the Apply verdict. **Defect D1 seen here.** |
| W11 first failure | `Open('DashboardPatchFail')`; `Run('All')`; `Snapshot`; `Shot 'W11'` | **Validate producer lands in T5b** (the chain's first stage). Real bake patch-gate failure. Bake FAIL; Apply/Verify/Package start counts stay zero; prior receipts retained |**PASS** 2026-09-06. `Open("DashboardPatchFail")`; `Run("All")` (runId 10). Real patch-gate failure: `P4 REFUSED target 'ContentToolNoSuchTarget' is not a Mesh in an_assault_assets_all.bundle - no Mesh named 'ContentToolNoSuchTarget' in unity=2019.4.31f1 assets=979 cldbTypes=320 ...`, terminal `ct_project: 1 FAILURE(S)`. Rows: Validate `never/pass/1`, Bake `stale/fail/1`, Apply/Verify/Package `never/none/**0**` - the chain stopped and no later stage was ever entered. `W11.png`. |
| W12 restart required | `Acceptance('resident')`; `Open('DashboardResident')`; `Run('All')`; `Snapshot`; `Shot 'W12'` | **Apply and Verify producers land in T5b**, and `Acceptance('resident')` with them. A really resident bundle: Apply PASS/S1 with the exact S1 text, Verify VOID/R30, no Package dispatch, no forced unload |**PASS** 2026-09-06. An AN Assault unit was stood in the bench first (`ct_bench unit AN_Assault1_CharacterTemplateDef`) so `BundleLive.ResidentNow("an_assault_assets_all.bundle")` -> **true**; before that `Acceptance("resident")` correctly refused (`the game has none of Replace_Leftleg's declared bundles loaded right now`). Then `Acceptance("resident")` ok, `Open("DashboardResident")`, `Run("All")` (runId 20-). Apply PASS with `installation: "restart required"` and the exact S1: `REFUSED: restart required: an_assault_assets_all.bundle is already loaded (as '35151a9626b11fde9909637b2e38b7a1.bundle'). Unity rejects a second bundle of the same identity, and unloading the game's copy would pull it out from under live objects. Restart, then enable 'acceptance.dashboardresident'.` Header `restartRequired=true`. Chain terminal in the log: `Verify: VOID - restart required for 'acceptance.dashboardresident'.` Package `starts=0` - never dispatched. No forced unload. `W12.png`. NOTE: the Verify ROW stays `never/none` (the stage was refused, never entered); the R30 text is in the log tail, per `chain.Terminal`. |
| W13 cancel | `Open('DashboardAuthor')`; `Acceptance('arm-cancel-bake')`; `Run('Bake')`; **poll** `Snapshot('')` until `barrierParked=true` AND `barrierRunId`==the `runId` `Run` returned; `Shot 'W13-armed'`; `Cancel`; **terminal poll** until `busy=false`; `Cancel` again; `Snapshot`; `Shot 'W13'` | **`DashboardAuthor`, never `DashboardValid`** — W10 installed the latter and R38 would refuse this bake before it ever reached the barrier. The barrier parks a WORKER, never the main-thread RPC pump, or the poll itself cannot be answered. The first poll observes THIS run parked (timeout = failed row); Cancel is the button's entry point; terminal poll ends R31/VOID with ONE receipt; later start counts zero; busy clears only after acknowledgement AND worker completion; the second Cancel adds nothing; previous outputs byte-identical |**PASS** 2026-09-06. `Open("DashboardAuthor")`; `Acceptance("arm-cancel-bake")`; `Run("Bake")` -> runId 12. Poll observed `barrierParked=true, barrierRunId=12, busy=true, stage=Bake` - and the poll itself was answered, so a WORKER is parked, not the RPC pump. `Cancel` -> `{"ok":true,"runId":12,"acknowledged":false}`; terminal poll ended `busy=false, cancelRequested=true, cancelAcknowledged=true`. ONE receipt, R31 verbatim: `ct_project: CANCELLED - 'DashboardAuthor' stopped before it published anything; the patched copies and this project's own bundle are exactly as they were.` Bake `never/void/starts=1`; Validate/Apply/Verify/Package `starts=0`. Second `Cancel` -> `{"ok":false,"runId":12,"acknowledged":false,"error":"nothing is running."}` - adds nothing. Nothing published: `...\Patched\d29f58a2\acceptance.dashboardauthor` did not exist after the cancel. `W13-armed.png`, `W13.png` (constant five-row layout, Cancel GREY). |
| W14 Failed block | `C 'ct_route7' @('apply','DashboardPatchFail')`; `Open('DashboardPatchFail')`; `Run('Apply')`; `Snapshot`; `Shot 'W14'` | **Apply producer lands in T5b**, with `LegacyDiskActive`/`WriteOutsideRoots` — R34/R36 are unreachable before it. Console setup really sets `Failed`; dashboard admission R29; Apply and `Run all` disabled, no retry, no clearing; Validate/refresh cannot clear the badge |**FAIL** 2026-09-06 - two findings. Setup worked: `ct_route7 apply DashboardPatchFail` -> `NOT APPLIED: patching the shipped bundle(s) reported 1 failure(s) ...`, and the header then carried `failedMember=acceptance.dashboardpatchfail` (the badge is painted, `W14.png`: `Session  Ready.  session block: acceptance.dashboardpatchfail`). R29 on Apply is exact: `Run("Apply")` -> `{"ok":false,"runId":0,"refusal":"'acceptance.dashboardpatchfail' failed to bake earlier in this session - not baking it again. Fix the lines it printed, then 'ct_route7 apply DashboardPatchFail'."}`. Validate did NOT clear the badge (`failedMember` still set after a Validate run). **(1) `Run("All")` was ADMITTED under the session block** -> `{"ok":true,"runId":17}`; the panel button is disabled (`LifecycleDashboard.cs:696`) but `LifecycleState.Admit` does not refuse `All`, so the seam and the button disagree - the row asks for both to be disabled. The chain did stop before Apply (Apply `starts=0`). **(2)** the panel half of the evidence is unreadable: **defect D1** pushes Apply/Verify/Package, Progress, `Run all`/`Cancel` and the Log tail off screen. |
| W15 restart proof | after restart + identity preflight: `Acceptance('enable-resident')`; `Snapshot`; `Open('DashboardResident')`; `Run('Verify')`; `Snapshot`; `Shot 'W15'` | **Verify producer and `Acceptance('enable-resident')` land in T5b.** Real enable callback (`Route7.Toggle`), fresh load-back, the **per-target** claim/path census of S6 (not `Holds`); a partially claimed fixture must produce VOID naming the unserved target; new-session `Failed` clear |**PASS** 2026-09-06, after a real process restart (new PID 12424). Identity preflight: `gameRoot=D:\PP-Instance2`, `runId=0`, **`failedMember` empty** - the new-session `Failed` clear. `Acceptance("enable-resident")` -> the REAL `Route7.Toggle(at, true)`. **Both arms measured.** (a) Partially claimed: `acceptance.dashboardauthor` held `an_assault_assets_all.bundle` from startup (see D5), so `Run("Verify")` gave `Verify: VOID - only 0 of 1 declared target(s) are served from this project's copies for 'acceptance.dashboardresident'; the target(s) named above are unproven.` while every load-back gate above it PASSed (P4/P4-ctl-shipped/P4-bytes/P5/P6) - the VOID is the per-target claim/path census of S6, not `Holds`. (b) Clean: `BundleLive.Uninstall("acceptance.dashboardauthor")`, `Acceptance("enable-resident")` again, `Run("Verify")` -> `Verify: PASS - load-back gates passed; 1 of 1 declared target(s) served from this project's copies for 'acceptance.dashboardresident'.` `W15.png`. |
| W16 stale | `Open('DashboardValid')`; `Acceptance('change-source')`; `Run('Verify')`; `Snapshot`; `Shot 'W16'` | **Verify producer and `Acceptance('change-source')` land in T5b.** **Stays on `DashboardValid` deliberately** — it needs W10's receipt to make stale, and `Verify` never rewrites, so R38 is unreachable here. Receipt becomes stale by an actual key comparison; no old PASS promoted, no automatic Apply. Run after W10 in the same process |**PASS** 2026-09-06, in the same process as W10. `Open("DashboardValid")` was still bound; `Acceptance("change-source")` ok; `Run("Verify")` -> `{"ok":false,"runId":0,"refusal":"Lifecycle: Verify blocked; patched copies is stale."}` - the receipt went stale by an actual `PatchCache.Key` comparison (`Route7.Observe`). Verify `starts` stayed **1**, the old PASS was not promoted, and no Apply was dispatched. `W16.png`. NOTE (**D6**): the ROW freshness word still reads `fresh` - it is only written at harvest, while `ctx.Copies` is recomputed per admission, so the panel says fresh while admission says stale. |
| W17 SHIP landing | `Acceptance('ship')`; `Snapshot`; `Shot 'W17'` | **Apply producer lands in T5b; `Acceptance('ship')` stays with Task 7**, which is what wires the Doctor handoff. A real successful SHIP opens Lifecycle after GUI dispatch, selects exactly `made.Root`, transfers the same Apply string and disposition, launches no duplicate bake/apply/package |**PASS** 2026-09-06. Driven through the REAL Doctor: tab -> MODEL DOCTOR, `FitBench.ShowPrototype(CHR_Human_Rig_Ready, Human)`, slot `Human_LeftLeg_SlotDef` -> `an_assault_assets_all.bundle` / `CHR_AN_Assault_M_Leftleg`, `TargetRefusal` null; `Doctor.PickTarget`, `Doctor.PickFile("D:\PP-Instance2\Mods\Replace_Leftleg\Content\Meshes\HeavyLeftLeg.glb")`; `ShipRefusal` **null**, `ShipSectionShowing` true, `DoctorShowing` true, `projectName` `Replace_CHR_AN_Assault_M_Leftleg`. `Acceptance("ship")` -> `{"ok":true,"scenario":"ship","gen":3}`. After the press: `FitBench.tab == 2` (LIFECYCLE opened outside the GUI event), `root=D:\PP-Instance2\Mods\Replace_CHR_AN_Assault_M_Leftleg` = exactly `made.Root`, `id=Replace_CHR_AN_Assault_M_Leftleg`, `shipPending=false`, `shipResult=applied - restart the game and enable 'Replace_CHR_AN_Assault_M_Leftleg' in the mod manager. Phoenix Point already loaded an_assault_assets_all.bundle.` Apply row `fresh/pass/installation="restart required"` and **`starts=0`** - the panel entered no stage; the other four rows are `never/none/0`, so no duplicate bake, apply or package. `Snapshot("s1s2")` carries the producer's own S1 line for the target. `W17.png`. |
| W18 console parity/package | `Open('DashboardAuthor')`; `Run('Validate')`; `Snapshot`; `C 'ct_project' @('DashboardAuthor')`; `Run('Bake')`; `Snapshot`; `Run('Package')`; `Snapshot`; `C 'ct_route7' @('verify','DashboardAuthor')`; `Run('Verify')`; `Snapshot`; `Shot 'W18'` | **Validate and Verify producers, and `ct_route7 verify` itself, land in T5b** — the parity half of this row cannot run before it. **`DashboardAuthor`** — this row bakes twice, which R38 forbids on the installed `DashboardValid`. Bake payload matches for the same unchanged project and key, **and matches the pre-extraction baseline captured at the head of Task 2, bytes and gate log** (this is Task 2's extraction proof, reaching it for the first time). Package matches its producer payload with `ok=true`, writes only a new external directory, previous package intact. **Verify parity: the console verb's terminal line and the dashboard's verdict are the same string character for character**, both out of the one producer; the console call installs and writes nothing; `dryrun/revert/stacktest` still print the unchanged removal text (`Route7.cs:57`–`:63`) |**PASS**, with ONE sub-claim VOID. 2026-09-06 on `DashboardAuthor`. Validate: `Validate: PASS - 'DashboardAuthor' - key 64746eddadd74af6acf4d2b88e37312f6121d9ae.` **Bake payload identical across the two doors:** SHA-256 of `...\Patched\d29f58a2\acceptance.dashboardauthor\an_assault_assets_all.bundle` = `8DAF8B094991D7D03C34491CB7B79D00342F470119842DDBF1362A259D29F992` before the console verb, after `ct_project DashboardAuthor`, and after the dashboard `Run("Bake")` - three measurements, one hash; the first 1200 chars of the two gate logs are byte-identical (the dashboard's is `Protocol.Clip`-truncated at 2000, so only a prefix is comparable over the wire). Package: `PACKAGED 3 file(s), 4270266 B into ...\Packages\acceptance.dashboardauthor\20260906-125621-5`, a NEW directory beside the untouched `20260906-124419-14` (every file hash of the previous package unchanged). **Verify parity, character for character:** `ct_route7 verify DashboardAuthor` terminal line and the dashboard's refusal are both `Verify: VOID - restart required for 'acceptance.dashboardauthor'.` (R30, armed because the bundle is genuinely resident and the claim is `acceptance.dashboardresident`'s after W15). The console call wrote nothing - the bundle hash was unchanged after it. Removal text unchanged for all three: `REMOVED: 'ct_route7 dryrun|revert|stacktest' wrote into your Phoenix Point installation and no longer exists - ContentTool never writes there. ...` `W18.png`. **VOID sub-claim:** "matches the pre-extraction baseline captured at the head of Task 2" - that baseline was written to a PREVIOUS session's scratchpad (plan:302-305) and is not on disk here, so it could not be compared; the two-door byte identity above is what this run proves. |
| W19a closed run, **worker-only** | `Open('DashboardAuthor')`; `Run('Package')`; close the bench with the chord while it runs; poll `Snapshot('')` until `busy=false`; reopen; `Snapshot`; `Shot 'W19a'` | `Package` is the one stage that is plain `System.IO` end to end (`Package.cs:15`), so it has NO main-thread final segment and can genuinely finish with the window closed. The run completes, receipt and log recorded, reopening SHOWS the terminal result without re-running. `Cancel` reachable while closed |**PASS** 2026-09-06. `Open("DashboardAuthor")`; `Run("Package")` -> runId 14; `ct_bench close` -> `ct_bench closed - the screen you came from was never left, so it is still there.` The RPC seam answered with the bench closed (`Cancel` -> `{"ok":false,"runId":14,...,"error":"nothing is running."}` - reachable), the run reached `busy=false`, and `Snapshot("Package")` served the terminal receipt **while the bench was still closed**: `PACKAGED 3 file(s), 4270266 B into ...\Packages\acceptance.dashboardauthor\20260906-124419-14`. Reopened -> identical verdict, `starts` still **1**, nothing re-run. `W19a.png`. CAVEAT: `Package` completed faster than the close landed, so "closed mid-run" is proven by the closed-window HARVEST and re-read, not by catching the worker in flight. |
| W19b closed run, **main-thread arm** | `Open('DashboardAuthor')`; `Run('Bake')`; close the bench while it runs; `Snapshot('')` **once** — assert `busy=true` and the parked-for-paint state, do NOT terminal-poll; reopen and let it paint; THEN poll until `busy=false`; `Snapshot`; `Shot 'W19b'` | v1's single row deadlocked by construction: it terminal-polled a run whose final phase waits for an open, painted panel, so `busy=false` could never arrive. Split here. Assertions: while closed the row reports it is waiting for a painted panel and nothing is re-run; after reopening it resumes and reaches its terminal result once |**PASS** 2026-09-06. Deviation from the wording, same observable: the bench was closed FIRST and then `Run("Bake")` was pressed (runId 16) - the bake finishes in under 4 s, so closing after the press is a race that cannot be won by hand. Single `Snapshot("")` while closed: `busy=true, stage=Bake, parkedForPaint=true`, Bake `starts=4` unchanged and the row still holding the previous PASS - it reports it is waiting for a painted panel and nothing is re-run. Held there for 13 s (second single poll: still `busy=true, parkedForPaint=true, starts=4`). Reopened on the LIFECYCLE tab -> the segment unparked and reached its terminal result ONCE: `busy=false, parkedForPaint=false`, Bake `pass`, `starts` still 4. `W19b.png`. No terminal-poll was issued while closed. |
| W20 competing producer | `Open('DashboardAuthor')`; `Acceptance('arm-cancel-bake')`; `Run('Bake')`; poll until parked; `C 'ct_project' @('DashboardAuthor')`; `Cancel`; poll until `busy=false`; `Snapshot`; `Shot 'W20'` | **`DashboardAuthor`** — an R38 refusal from the installed `DashboardValid` would mask the R37 this row exists to prove. The console verb hits R37 and returns immediately, writing nothing — no second bake, no key stamped over the parked run's copies. After the cancel the claim is released and a plain `C 'ct_project'` succeeds |**FAIL** 2026-09-06 - R37 never fired. `Open("DashboardAuthor")`; `Acceptance("arm-cancel-bake")`; `Run("Bake")` -> runId 13; poll observed `barrierParked=true, barrierRunId=13, claimHeld=null`. `ct_project DashboardAuthor` then ran a **full bake to completion** - `WROTE ...\acceptance.dashboardauthor\an_assault_assets_all.bundle 17967951 B`, `ct_project: ALL PASS` - i.e. a second bake and a key stamped over the parked run's project, which is exactly what this row exists to forbid. Root cause: `Barrier.Wait(id)` is the FIRST statement on the worker (`LifecycleJob.cs:157-159`, "parked before it has touched anything"), and the `OutputClaim` R37 guards is taken inside `ProjectBake.Run` (`ProjectBake.cs:88-93`, `OutputClaim.cs:10`, `:47-50`) - so at the armed point NO claim is held and R37 is unreachable by construction. The invariant itself is not disproved (one owner per output directory still holds once a bake starts); the ACCEPTANCE RECIPE targets the wrong window. The rest of the row passed: `Cancel` -> terminal, Bake `void` with the R31 receipt, and a plain `ct_project DashboardAuthor` after the cancel -> `ct_project: ALL PASS`. `W20.png`. |

---

**ADDENDUM — the `f1d6605` re-check, 2026-09-06.** Same bench, a NEW build: ContentTool rebuilt at
HEAD `dda82dd` (fix commit `f1d6605`) by the repo's own `deploy.ps1` →
`D:\PP-Instance2\Mods\ContentTool\ContentTool.dll` SHA-256 `1C73192188E1D10F769B3F4B41DA18FAB3D368DD65F4F0CA5B5102784BC7FECC`,
byte-identical to `bin\Release\ContentTool\ContentTool.dll`, built 13:13:25 — after the fix sources
(`LifecycleState.cs` 13:07:22, `LifecycleDashboard.cs` 13:08:14). PPBridge was NOT redeployed
(`build=3068ae67` in every reply, no `stale:true`); `ppcli-enabled` was re-created for the run and
deleted at exit. The profile (`…\Steam\76561197996210592\Options.jopt`) was **never edited** — it holds
exactly `Replace_Leftleg`, `com.morgott.ContentTool`, `com.morgott.Renderforge`, `com.morgott.PPBridge`
and **zero** `acceptance.*` entries. Two hand launches (`Start-Process … -mods -logFile
D:\PP-Instance2\ct-t8-recheck.log`, PID 27480; `…-recheck2.log`, PID 37772 for the D5 restart half),
each gated on `connect state` answering first; identity preflight `gameRoot=D:\PP-Instance2` on the
seam. Geoscape from `plans\start-campaign.json` (37 steps, 14.7 s), then `ct_bench open` + `FitBench.tab = 2`.
Screenshots in this session's scratchpad (`D1-oneline.png`, `W14-recheck.png`, `D6-refresh.png`; each
also wrote a `.scene.png` — the reply's own note says the IMGUI is in the PRIMARY file, the `.scene.png`
holds only the pre-upscale 3D frame).

**Result: 5 of 5 rows PASS.**

| Row | What was run | Result |
|---|---|---|
| **W14** (D2) | `C 'ct_route7' @('apply','DashboardPatchFail')` → `NOT APPLIED: patching the shipped bundle(s) reported 1 failure(s) …`; `Open('DashboardPatchFail')` → header `failedMember=acceptance.dashboardpatchfail`; `Run('Apply')`; `Run('All')` | **PASS.** `Run("All")` is now **REFUSED** with the block's own R29 word — `{"ok":false,"runId":0,"refusal":"'acceptance.dashboardpatchfail' failed to bake earlier in this session - not baking it again. Fix the lines it printed, then 'ct_route7 apply DashboardPatchFail'."}` — character-identical to what `Run("Apply")` returns, so no new refusal string (`REFUSAL-COUNT` stays 17). Was `{"ok":true,"runId":17}`. `W14-recheck.png`: `Run all` GREY, Apply's `Run` GREY, badge `session block: acceptance.dashboardpatchfail` painted, all five rows on screen — the seam and the button now say the same thing. |
| **D1** | `Open('DashboardAuthor')`; `Run('Bake')` → runId 2, terminal `Bake pass`, header **`verdictLength 1225451`**; `Shot 'D1-oneline'` | **PASS.** The Bake row draws ONE logical line ending in ` ...`: `project 'acceptance.dashboardauthor' at D:\PP-Instance2\Mods\DashboardAuthor: 0 texture(s), 1 mesh(es), 0 model(s), 0 video(s), 0 sound(s), 1 replacement(s) ...`. From a 1 225 451-char verdict the panel still shows all five stage rows, `Progress`/`publishing 0/0`, `Run all` + `Cancel` and the full `Log tail` scroll box — the layout that D1 destroyed. The wire is untouched: `verdictLength` still reports the whole 1 225 451 and the tail still carries the bake log verbatim. NOTE: `RowRoom` is 160 chars, and IMGUI soft-wraps that one line across three display lines in the ~380 px column — bounded, constant, and not the unbounded growth D1 was about. A refused-target bake (`DashboardPatchFail`, W11-style) was measured too: `verdictLength 1399`, same one-line treatment. |
| **D3** | the same Bake, `message` read by reflection at three points | **PASS.** Immediately after `Run('Bake')`: `message = "Queued: Bake"`. After the run reached `busy=false`: `message = null` — the pump clears it at harvest, and the panel line beside `Run all`/`Cancel` reads `—` (`D1-oneline.png`). A refusal SURVIVES: after `Run('Apply')` and after `Run('All')` under the session block, `message` holds the R29 text and `W14-recheck.png` shows it painted beside the two disabled buttons. |
| **D5** | `Acceptance('prepare')`, then `Acceptance('resident')`; each fixture's `meta.json` read off disk; then a real process restart and a startup claim census | **PASS, both halves.** Distinct ids, one per fixture: `DashboardValid` → `"ID": "acceptance.dashboardvalid"`, `DashboardPatchFail` → `"acceptance.dashboardpatchfail"`, `DashboardAuthor` → `"acceptance.dashboardauthor"`, `DashboardResident` → `"acceptance.dashboardresident"` — written through `ProjectScaffold.Meta(modId)`, `Name` and `Dependencies` composed the same way a real project's are. **Startup half:** at THIS session's FIRST launch the leftover pre-fix fixtures still declared `Replace_Leftleg`, and `ct_route7` showed `acceptance.dashboardauthor -> an_assault_assets_all.bundle` claimed by a fixture nobody enabled — D1/D5's own symptom, observed live one last time. After the four forks and a restart, the same census reads `Replace_Leftleg -> an_assault_assets_all.bundle` and **no `acceptance.*` redirection at all**: only the mod actually in `MOD_ACTIVATED` was enabled, and W10's first-attempt VOID / W15's contested arm cannot recur. |
| **D6** | `Open('DashboardAuthor')`; `Run('Bake')` → all five rows `fresh`; `Acceptance('change-source')`; **no Run** — the Refresh button's own assignment `rescan = true` (`LifecycleDashboard.cs:657`) set through the seam; `Snapshot('')`; `Shot 'D6-refresh'` | **PASS.** Before the press, after the source bytes changed: `Validate=fresh Bake=fresh Apply=fresh Verify=fresh Package=fresh` (no measurement point had passed). After the press, with no stage entered: `Validate=stale Bake=stale Apply=stale Verify=stale Package=stale` — `Drain`'s refresh branch (`:747`) ran the one `Freshen` (`:948`) that writes `ctx.Copies` AND every row. W16's split ("the panel says fresh while admission says stale") is gone. `D6-refresh.png` shows the five stale rows over the still-bounded Bake verdict. The Verify admission this project can reach is the higher-precedence R30 (`Verify: VOID - restart required for 'acceptance.dashboardauthor'.`), so the stale word itself was read off the rows, not off a second refusal. |

**Bench state at exit:** Instance2 stopped (path-filtered `Where-Object { $_.Path -like 'D:\PP-Instance2\*' }`),
no `PhoenixPointWin64` process anywhere, `D:\PP-Instance2\Mods\PPBridge\ppcli-enabled` deleted,
`ct-acceptance-enabled` left in place. The four `Dashboard*` fixtures were re-forked by this run and now
each carry their own `meta.json`; `Mods\Replace_Leftleg` is enabled again (the `resident` scenario's own
toggle), which is the profile's declared state.
