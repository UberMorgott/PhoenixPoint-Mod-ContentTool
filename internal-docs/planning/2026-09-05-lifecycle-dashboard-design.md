...
| S8 | Verify PASS, empty census | `Verify: PASS - nothing to verify for '{name}'; this project declares no patched target - its row(s) are served live by ct_video.` | NEW; a project with no declared bundle row has no patched copy and never gets a key (`Route7.cs:157`/`:163` leave `wantReplace` false), so §4.6's Verify row admits it instead of refusing it with R28 forever |
...
`barrierParked` is published only once a worker is ACTUALLY parked, never on arming alone, or W13's first poll passes
