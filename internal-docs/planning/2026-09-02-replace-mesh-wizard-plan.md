...
- [x] **Step 3: Record the evidence, then commit.** Fill this table in, in this file, with what the run actually
  ...
  **GAP 2 CLOSED, 2026-09-06.** Root cause was the rep collapse in `PrototypeHarvest.Read` (old `:93-103`): one
  ...
  - STILL OWED IN GAME (`ShippedTarget` is not test-linked, W4 `:2079`): open the browser on
    ...
    **PAID, 2026-09-06 - see "gap 2 in-game proof" below.**

  **GAP 2 IN-GAME PROOF, 2026-09-06.** ContentTool at HEAD `8b16756` (fix commit `bfaf4a1`), `dotnet build -c
  Release` → `Ошибок: 0` / `Предупреждений: 0`, deployed by the repo's own `deploy.ps1` (default target
  `D:\PP-Instance2`, the T8 recipe verbatim) → `D:\PP-Instance2\Mods\ContentTool\ContentTool.dll` **1 973 760 B,
  06.09.2026 16:15:56**. PPBridge NOT redeployed - `build=3068ae67` in every reply, no `stale:true` anywhere; only
  `D:\PP-Instance2\Mods\PPBridge\ppcli-enabled` was created (deleted again at exit). Launched BY HAND
  `Start-Process 'D:\PP-Instance2\PhoenixPointWin64.exe' -ArgumentList '-mods','-logFile','D:\PP-Instance2\ct-gap2.log'`
  (PID 1284); the gate `connect state -PPRoot 'D:\PP-Instance2' -ProfileId 76561197996210592` answered
  `{"phase":"menu","scene":"HomeScreen","levelState":"Playing"}` before anything else was sent. Geoscape from
  `plans\start-campaign.json` (37 steps, 12.5 s), then `ct_bench open` and the static seams
  `FitBench.Prototypes()` / `ShowPrototype` / `SlotTargets()` / `ShownVariant()` / `FitBench.Doctor` through
  `connect call` + `connect multi`. Screenshots in this session's scratchpad (`gap2-*.png`; every capture also wrote
  a `.scene.png` - the IMGUI is in the primary file). **All five items PASS.**

  | id | Check | Evidence |
  |---|---|---|
  | G1 | The catalogue lists the armour variants beside `Human` | **PASS.** `FitBench.Prototypes()` → 36 records. The `CHR_Human_Rig_Ready` record's `DisplayName` is still the bare **`Human`** and it now carries **166 variants** (`Variants.count`), every one named `Human / <rep>`: `Human / AN_Assault1`, `Human / AN_Assault2_1`, … `Human / PX_AssaultStarting`, **`Human / PX_HeavyStarting`**, `Human / PX_Heavy_Berserker_L7_TacCharacterDef`, `Human / PX_Chitin_TacCharacterDef`, … (all 166 read live, `failed=0`, kept as `gap2-variant-names.txt`). 20 of the 36 records now hold more than one variant - `CHR_Human_Rig_Ready` 166, `ALN_Crabman_Rig_Ready` 58, `ALN_Chiron_Rig_Ready` 19, `ALN_Fishman_Rig_Ready` 17, `ALN_Queen_Rig_Ready` 17, `ALN_Mutog_Rig_Ready` 8, `ALN_Siren_Rig_Ready` 8, `ALN_Acheron_V01_Rig_Ready` 6, `VEH_NJ_Armadillo_Rig_Ready` 4, `WPN_PX_Injector_V01_Rig_Ready` 4, `VEH_SYN_Sanator_Rig_Ready` 4, `ALN_Acidworm_Rig_Ready` 3, `ALN_Egg_FireWorm_Rig_Ready` 3, `CHR_AC_MediumGuardian_Rig_Ready` 3, `CHR_NJ_TEC_Turret_T01_V01_Rig_Ready` 3, `ALN_Swarmer_V01_Rig_Ready` 3, `ALN_Facehugger_Rig_Ready` 2, `CHR_AC_1x1_Guard_Rig_Ready` 2, `VEH_KS_Death_Buggy_05_Rig_Ready` 2, `VEH_PX_Scarab_V01_Rig_Ready` 2; **350 variants over 36 records** (was 36). Panel proof `gap2-catalog-human.png`: the browser row reads `v Human   110 bindable bone(s), 166 varian…` over `* Human / AN_Assault1   9 slot(s)   on the platform`. **NAME CORRECTION:** the plan's guess `Human / PX_Heavy1` does not exist - `PX_Heavy1_CharacterTemplateDef` IS in the repository (`connect find` → 1 def) but it shares a template bodypart SET with a lower-ordinal def and merges into that variant, exactly as `PrototypeCatalog.Representatives` specifies. The PX heavy armour set is reachable as **`Human / PX_HeavyStarting`** (`RepresentativeCharacter = PX_HeavyStarting_CharacterTemplateDef`). |
  | G2 | Stand the PX heavy variant up; a body slot resolves to `px_heavy_assets_all.bundle`, `TargetRefusal = null`, verdict BY NAME | **PASS.** `FitBench.ShowPrototype(<CHR_Human_Rig_Ready>, <Human / PX_HeavyStarting>)` returned `null` (no refusal); `ShownVariant().Name` read back **`Human / PX_HeavyStarting`**. `SlotTargets()` → **10 targets**, `failed=0` over 40 reads: `Human_Torso_SlotDef` → **`px_heavy_assets_all.bundle` / `CHR_PX_HVY_TS_M_V01`**, `Human_Head_SlotDef` → `px_heavy_assets_all.bundle` / `CHR_PX_HVY_HG_M_V01`, `Human_LeftArm_SlotDef` → `…/CHR_PX_HVY_LA_M_V01`, `Human_RightArm_SlotDef` → `…/CHR_PX_HVY_RA_M_V01`, `Human_LeftLeg_SlotDef` → `…/CHR_PX_HVY_LL_M_V01`, `Human_RightLeg_SlotDef` → `…/CHR_PX_HVY_RL_M_V01`, plus `Heavy_Jetpack_SlotDef` → `px_equipment_assets_all.bundle` / `WPN_PX_Jump_Pack_V01`. **`TargetRefusal` is `null` on every one of the ten** (raw reply `{"ok":true,"value":null}` re-read for `Human_LeftLeg_SlotDef`); the three that carry no pair (`Human_FacialHair_SlotDef`, `Human_Hair_SlotDef`, `Human_Legs_SlotDef`) produced no renderer, which is the documented `Unavailable` path and not a refusal. Verdict: `FitBench.Doctor.PickTarget(<Human_LeftLeg_SlotDef target>)` + `PickFile("E:\DEV\PhoenixPoint\ContentTool\APOCD GLBs for content tool without apply tranforms\CHR_PX_HVY_LL_M_V01_0fa9bde0c679e665.glb")` → `Ready.Outcome` = **`ByName`**, `Ready.Failure` = `null`. Panel proof `gap2-pxheavy-doctor.png`: a **PX Heavy** soldier on the platform (not AN Assault), `12599 verts, 11555 tris, 10 joints, 4 influence(s)/vertex`, `BY NAME - your weights will be used`, and the SHIP row pre-filled `project Replace_CHR_PX_HVY_LL_M_V01   target px_hea…`. This is the pair the old rep collapse made unreachable. |
  | G3 | The rig census line still says `MATCHES` | **PASS**, verbatim from `D:\PP-Instance2\ct-gap2.log`: `[Mods] [com.morgott.ContentTool] ct_bench prototypes: 46 manager(s) [census 46], 42 with a rig [42], 37 distinct rig(s) [37], 2551 transform(s) [2551], 36 binding prototype(s) [36] - MATCHES rig-census-2026-09-02.json.` 46/42/37/2551/36, all unchanged, with 350 variants behind them. |
  | G4 | No regression on the plain human variant, and the `Replace_Leftleg` fixture's target still resolves BY NAME | **PASS.** `ShowPrototype(…, <Human / AN_Assault1>)` → `null`, `ShownVariant().Name` = `Human / AN_Assault1`, `SlotTargets()` → 9 targets, `failed=0` over 36 reads, every pair identical to W4's record: `Human_LeftLeg_SlotDef` → **`an_assault_assets_all.bundle` / `CHR_AN_Assault_M_Leftleg`**, `TargetRefusal = null`; also `…_Torso` → `CHR_AN_Assault_M_Torso`, `…_Head` → `CHR_AN_Assault_M_Helmet`, `…_LeftArm` / `…_RightArm` / `…_RightLeg` → the matching `CHR_AN_Assault_M_*`. Fixture: `PickTarget(<Human_LeftLeg_SlotDef>)` + `PickFile("D:\PP-Instance2\Mods\Replace_Leftleg\Content\Meshes\HeavyLeftLeg.glb")` → `Ready.Outcome` = **`ByName`**, `Failure` = `null`. Panel proof `gap2-anassault-doctor.png`: `source HeavyLeftLeg.glb`, `12599 verts, 11555 tris, 11 joints, 4 influence(s)/vertex`, `BY NAME - your weights will be used`, target `an_ass…`. |
  | G5 | Bench open time and log noise from the per-rep reads | **PASS.** `ct_bench open` **0.93 s** wall clock, client to client (`791 unit template(s) … 201 weapon(s)`). The catalogue scan - the part gap 2 made per-representative - is `ct_bench rescan`, measured **0.28 s / 0.34 s / 0.25 s** over three cold scans (`harvest = null` each time), each one re-reading 46 managers, 37 rigs, 2551 transforms and running `ReadSlots`/`ReadClips`/body-state for **350** representatives instead of 36. **No spam:** the whole session's log holds **30** `com.morgott.ContentTool` lines in **1542** lines total, and a scan writes exactly **ONE** line - two extra rescans produced a delta of exactly 2. `ShippedTarget` derivation logging is per resolved slot and nothing else: 92 lines, 13 `resolved` lines = the 7 pairs of `Human / PX_HeavyStarting` + the 6 of `Human / AN_Assault1`. The only exception in the run is 13 x `ArgumentException: Mesh can not have more than 65000 vertices` from `UnityEngine.UI.Text.UpdateGeometry` / `CanvasUpdateRegistry.PerformUpdate`, all of them BEFORE `ct_bench open` (first at log line 704, the bench opened at 1215) - the game's own uGUI, nothing of this mod's on the stack. |

  **Observation, not a gap-2 defect.** After re-pointing the Doctor from the PX heavy slot to the AN Assault slot,
  the SHIP row's project name box still read `Replace_CHR_PX_HVY_LL_M_V01` while the target label had already moved
  to `an_ass…` (`gap2-anassault-doctor.png`). The default is seeded once and not re-seeded when the target changes,
  so an author who switches variants can ship a folder named after the previous target. Pre-existing behaviour of
  the SHIP row, untouched by `bfaf4a1`; recorded here because this run is where it was seen.

  **Bench state at exit.** `ct_bench close` answered `ct_bench closed - the screen you came from was never left, so
  it is still there.`; the Instance2 process was stopped path-filtered
  (`Get-Process PhoenixPointWin64 | Where-Object { $_.Path -like 'D:\PP-Instance2\*' } | Stop-Process`, never by
  name) and `D:\PP-Instance2\Mods\PPBridge\ppcli-enabled` deleted. Nothing was written into
  `D:\PP-Instance2\Mods\`, no project was created, the profile `…\Steam\76561197996210592\Options.jopt` was NOT
  edited, and the campaign started for the run was never saved. `D:\PP-Instance2\ct-gap2.log` left in place as the
  run's log. No PPCLI defect was hit, so nothing was appended to `PPCLI\ISSUES.md`.
  ...
  **OPEN GAP 1 — hashed extract joints — FIXED, `a2aee68`.** The sentence above ("an extract cannot be fed
  ...
  | **P4-bytes re-check a75162b** | ADDENDUM 2026-09-05, after the run above. The counted arm `P4-bytes` (`src\Bake\ProjectBake.cs` ~`:1708`, sha256 over `MeshFields.Buffers` via `BundleBaker.ReadMeshBuffers`; equal bytes = FAILURE) re-run in game on the SAME project. ContentTool HEAD `a75162b`, `dotnet build -c Release` → `Ошибок: 0`, `deploy.ps1` (default `D:\PP-Instance2`) → `D:\PP-Instance2\Mods\ContentTool\ContentTool.dll` 1 908 736 B, 05.09.2026 6:30:01. Instance2 launched BY HAND with `-mods`; gate answered `{"phase":"menu","scene":"HomeScreen"}`. Re-bake FORCED by deleting `…\ContentTool\Patched\d29f58a2\Replace_Leftleg\ct-cache.key` (40 B) so the cache could not short-circuit it, then `connect console '{"command":"ct_project","args":["Replace_Leftleg"]}'` (exit 0, `build=97b11b25`, `truncated:false`). | **PASS**, verbatim: `P4 PASS mesh 'CHR_AN_Assault_M_Leftleg' in the copy IS heavyleftleg -> verts=12599 indices=34665 format=UInt16 centre=-0.047,0.352,-0.02 extent=0.143,0.353,0.148 submeshes=1 vertexBytes=806336 streamPath=''` · `P4-ctl-shipped PASS the shipped an_assault_assets_all.bundle's 'CHR_AN_Assault_M_Leftleg' still has its own geometry -> verts=1886 indices=9318 format=UInt16 centre=-0.077,0.391,0.018 extent=0.085,0.395,0.123 submeshes=1 vertexBytes=150880 streamPath=''` · `P4-bytes PASS mesh 'CHR_AN_Assault_M_Leftleg' in the copy carries DIFFERENT vertex/index bytes than the shipped an_assault_assets_all.bundle -> ccdf4b161f6149f9edc6485a82f694e8e4476884d477f8121b66f07807acebe2 vertexBytes=806336 indexBytes=69330` · `ct_project: ALL PASS - this project has no bundle of its own; the patched copy(ies) above are the whole output`. No exception text anywhere in the reply; P1/P5/P6 unchanged (`P1 VOID`, `P5 PASS`, `P6 PASS`). Nothing new left on disk — the deleted `ct-cache.key` was rewritten by the successful bake. |
