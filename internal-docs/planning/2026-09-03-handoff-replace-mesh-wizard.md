...
- cavecrew review of `13e6361` (Task 1): CLEAN on name rules / manifest-first meta / MetaRefusal dependency check; one 🟡 test-only note — `ProjectScaffoldTests.Template()` (`:146-151`) does not JSON-escape the id it writes (prod `Meta()` does, via JsonWriter). Fold a `com.test"quote` id fixture into Task 2's arms; not a blocker. Task 1 = CLOSED.
