...
Choose the donor by weapon class. Phoenix Point selects hold poses and firing animation sets from
the weapon's tags; a pistol cloned from a rifle is still handled like a rifle.
...
```text
ct_weapon PASS 'Vulture PDW' (Morgott_VulturePDW_WeaponDef) cloned from PX_LaserPDW_WeaponDef; icon ok; prefab load started for key c7a9f1d24b6e4a3c8f5b7d1e9a2c4b60; tuning dmg 40->60 spread 2->3 range 20->13 (source intact)
```
...
`fit: auto` uniformly scales and offsets the model into the donor weapon's bounds. It cannot know
which end is the muzzle; add `flip: true` when the fitted direction is reversed.
...
reload, holster, shell effect, damage type and ammunition behavior.
...
- A model does not bring actor hold/firing animations; the donor's class and tags select them.
