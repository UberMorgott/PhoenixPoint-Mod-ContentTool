...
`ct_project` + `ct_route7 apply` are the SHIPPING loop and both need a restart. The dev workbench
writes the same slots into what is already on screen — the game keeps running, the author keeps
